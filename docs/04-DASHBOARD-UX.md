...
├──────────────────────────────────────────────────────────────┤
│  ✔ 497 صفحة قُرئت بنجاح          ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓░  96%    │
│  ⚠ 18 صفحة جودة سكانر منخفضة                    [عرض]        │
│  ✖ 3 صفحات لم يمكن استخراج النص منها             [عرض]        │
│  👁 2 صفحة قُرئت لكنها قد لا تُعرض بشكل صحيح      [قارن]       │
...
- **الصفحات المشبوهة بصريًا فئة ثالثة مستقلة** — لا تُجمع مع «قُرئت» ولا مع «لم تُقرأ»
  ([Document Processing §3.1](./01-DOCUMENT-PROCESSING.md)). زر `[قارن]` يفتح
  **صورة الصفحة بجانب النص المستخرج**:

```
┌─────────────────────────────┬─────────────────────────────┐
│  ما يظهر في الملف            │  ما قرأه النظام              │
│  ┌───────────────────────┐  │  محضر اجتماع رقم ١٤          │
│  │  ▪  ▪    ▪   ▪        │  │  اجتماع التنسيق الفني        │
│  │    ▪   ▪    ▪    ▪    │  │  ١. أنظمة الأمن              │
│  └───────────────────────┘  │  أكد المقاول أن عدد…         │
│                             │                              │
│  [الملف معطوب — اطلب نسخة أخرى]   [القراءة صحيحة — تابع]   │
└─────────────────────────────┴─────────────────────────────┘
```

  المستخدم يحسم في ثانيتين ما لا يحسمه أي وصف نصي. وقراره يُخزَّن ويرفع/يخفض ثقة الصفحة.

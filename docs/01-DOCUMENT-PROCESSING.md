...
كثير من مستخرجات PDF (وبعض محركات OCR) تُعيد العربية **بالترتيب البصري** وبـ **Arabic Presentation
Forms** بدل الحروف القياسية. النص الناتج يبدو عربيًا لكنه لا يطابق أي بحث:

```
المكتوب في المستند:  محضر اجتماع رقم ١٤
ما يعيده المستخرِج:   ﺮﻀﺤﻣ ﻉﺎﻤﺘﺟﺍ ﻢﻗﺭ ١٤        ← U+FB50–U+FEFF، ترتيب معكوس
```

**الكشف:** إذا تجاوزت نسبة المحارف في نطاقات `U+FB50–U+FDFF` أو `U+FE70–U+FEFF` عتبة 5% من
محارف الكتلة، فالمستخرِج يعمل بالوضع البصري.

**المعالجة:** تطبيع NFKC لإعادة الحروف إلى صيغتها القياسية، ثم عكس ترتيب المقاطع RTL لاستعادة
الترتيب المنطقي — قبل الفهرسة وقبل أي مطابقة. التفاصيل في §10.1.

> **هذا ليس عيبًا نادرًا.** ظهر على أول مستند عربي في أول حزمة اختبار
> (انظر `tests/fixtures/`). أي بحث عربي على مستند غير مطبَّع يعيد **صفر نتائج بصمت** —
> وهو أسوأ من الخطأ، لأنه يبدو كـ "لا يوجد شيء" لا كـ "فشلت القراءة".
...
| 7 | الأرقام العربية-الهندية → لاتينية | `١٢٠ مم` | `120 mm` |
